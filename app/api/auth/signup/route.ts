...
import { SESSION_COOKIE_NAME, SESSION_MAX_AGE_SECONDS, createSessionToken, hashPassword, hashSessionToken } from "@/lib/auth";
...
export async function POST(request: Request) {
  try {
    ...
    const name = typeof body.name === "string" && body.name.trim().length > 0 ? body.name.trim() : "Runner";
